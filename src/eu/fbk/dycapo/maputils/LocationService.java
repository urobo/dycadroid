		...
		try {
			DycapoServiceClient.callDycapo(
					...
					DycapoServiceClient.uriBuilder("persons/"
							+ usr.getUsername() + "/location"),
					loc.toJSONObject(), usr.getUsername(), usr.getPassword());
	...
	public static final eu.fbk.dycapo.models.Location getPosition(Person p) {
		...
		try {
			JSONObject json = DycapoServiceClient.callDycapo(
					...
					DycapoServiceClient.uriBuilder("persons/" + p.getUsername()
							+ "/location"), null, usr.getUsername(),
					usr.getPassword());
			eu.fbk.dycapo.models.Location ploc = DycapoObjectsFetcher
					.buildLocation(json);
			return ploc;
		} catch (DycapoException e) {
			Log.e(TAG, e.getMessage());
