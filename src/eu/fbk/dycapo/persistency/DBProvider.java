...
public class DBProvider {
	...
	public static ObjectContainer provideDB() {
		return Db4o.openFile(DBProvider.configure(),
				DBProvider.db4oDBFullPath());

	}

	public static ObjectContainer getDatabase() {
		if (database == null)
			database = DBProvider.provideDB();
		return database;
	}
	...
	private static Configuration configure() {
		...
		dbConfiguration.objectClass(Trip.class).objectField(Trip.HREF)
				.indexed(true);
		dbConfiguration.objectClass(Trip.class).cascadeOnUpdate(true);
		dbConfiguration.objectClass(Trip.class).cascadeOnDelete(true);

		dbConfiguration.objectClass(Location.class)
				.objectField(Location.GEORSS_POINT).indexed(true);
		...
		dbConfiguration.objectClass(Person.class).objectField(Person.USERNAME)
				.indexed(true);
		...
		dbConfiguration.objectClass(Mode.class).objectField(Mode.LIC)
				.indexed(true);
		dbConfiguration.objectClass(Mode.class).cascadeOnDelete(true);
		dbConfiguration.objectClass(Mode.class).cascadeOnUpdate(true);

		dbConfiguration.objectClass(User.class).objectField(Person.USERNAME)
				.indexed(true);
		...
		dbConfiguration.objectClass(ActiveTrip.class)
				.objectField(ActiveTrip.HREF).indexed(true);
		...
		dbConfiguration.objectClass(Participation.class)
				.objectField(eu.fbk.dycapo.models.Participation.AUTHOR)
				.indexed(true);
