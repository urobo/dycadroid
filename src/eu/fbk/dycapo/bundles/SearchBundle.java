...
public abstract class SearchBundle {
	...
	public static final Bundle toBundle(Search search){
		...
		if (search.getAuthor() instanceof Person)
			data.putBundle(Search.AUTHOR, PersonBundle.toBundle(search.getAuthor()));
		
		if (search.getOrigin() instanceof Location)
			data.putBundle(Search.ORIGIN, LocationBundle.toBundle(search.getOrigin()));
		
		if (search.getDestination() instanceof Location)
			data.putBundle(Search.DESTINATION, LocationBundle.toBundle(search.getDestination()));
	...
	public static final Search fromBundle(Bundle data){
		...
		if (data.containsKey(Search.AUTHOR))
			search.setAuthor(PersonBundle.fromBundle(data.getBundle(Search.AUTHOR)));
		
		if (data.containsKey(Search.ORIGIN))
			search.setOrigin(LocationBundle.fromBundle(data.getBundle(Search.ORIGIN)));
		
		if (data.containsKey(Search.DESTINATION))
			search.setDestination(LocationBundle.fromBundle(data.getBundle(Search.DESTINATION)));
		
		if (data.containsKey(Search.TRIPS)){
			Bundle temp = data.getBundle(Search.TRIPS);
			Iterator<String> i = temp.keySet().iterator();
			List<Trip> trips = new ArrayList<Trip>();
			while(i.hasNext())trips.add(TripBundle.fromBundle(data.getBundle(i.next())));
