...
public final class DirectionsResponseParser {
	...
	private static final String[] KEYS = { "status", "routes",
			"overview_polyline", "duration", "points", "value", "legs", "steps" };

	private static final String[] RESPONSE_STATUS = { "OK", // indicates the
															// response contains
															// a valid result.
			"NOT_FOUND", // indicates at least one of the locations specified in
							// the requests's origin, destination, or waypoints
			...
			"MAX_WAYPOINTS_EXCEEDED", // indicates that too many waypoints were
										// provided in the request The maximum
										// allowed waypoints is 8, plus the
										// origin, and destination. ( Google
										// Maps Premier customers may contain
	...
	private static final String[] ERROR_MESSAGES = { "OK",
			...
			"Service Rejected the Request", "Unknown Error" };
	...
	private static String resolveStatus(int key) {
		if (key >= 0 && key < RESPONSE_STATUS.length)
			return RESPONSE_STATUS[key];
		return null;
	}

	private static String resolveErrorMessage(int key) {
		if (key >= 0 && key < ERROR_MESSAGES.length)
	...
	public static void parseDirections(InputStream is) throws DycapoException {
		...
		try {
			...
			if (status.equals(resolveStatus(OK))) {
				...
				JSONObject overviewPolyline = mainRoute
						.getJSONObject(getKey(OVERVIEW_POLYLINE)); // getting
																	// the
																	// polyline
				...
				for (int i = 0; i < size; i++) {
					totalDurationSecs += legs.getJSONObject(i)
							.getJSONObject(getKey(DURATION))
							.getInt(getKey(VALUE)); // getting total duration
				}

				ActiveTrip aTrip = DBTrip.getActiveTrip();
				Route mRoute = new Route();

				mRoute.setmDecodedPolyline(PolylineDecoder
						.decodePolyline(overviewPolyline
								.getString(getKey(POINTS))));
