...
public abstract class TripBundle {
	...
	public static final Bundle toBundle(Trip trip) {
		...
		if (trip.getAuthor() instanceof Person)
			result.putBundle(Trip.AUTHOR,
					PersonBundle.toBundle(trip.getAuthor()));

		Bundle locations = new Bundle();

		if (trip.getOrigin() instanceof Location)
			locations.putBundle(Location.POINT_TYPE[Location.ORIG],
					LocationBundle.toBundle(trip.getOrigin()));

		if (trip.getDestination() instanceof Location)
			locations.putBundle(Location.POINT_TYPE[Location.DEST],
					LocationBundle.toBundle(trip.getDestination()));

		int size = trip.getWaypoints().size();
		for (int i = 0; i < size; i++)
			locations.putBundle(
					Location.POINT_TYPE[Location.WAYP] + String.valueOf(i),
					LocationBundle.toBundle(trip.getWaypoints().get(i)));
		...
		if (result.containsKey(Trip.PREFERENCES))
			result.putBundle(Trip.PREFERENCES,
					PrefsBundle.toBundle(trip.getPreferences()));
	...
	public static final Trip fromBundle(Bundle data) {
		...
		if (data.containsKey(Trip.LOCATIONS)) {
			...
			if (locations.containsKey(Location.POINT_TYPE[Location.ORIG]))
				trip.setOrigin(LocationBundle.fromBundle((locations
						.getBundle(Location.POINT_TYPE[Location.ORIG]))));

			if (locations.containsKey(Location.POINT_TYPE[Location.DEST]))
				trip.setDestination(LocationBundle.fromBundle(locations
						.getBundle(Location.POINT_TYPE[Location.DEST])));

			int i = 0;
			while (locations.containsKey(Location.POINT_TYPE[Location.WAYP]
					+ String.valueOf(i))) {
				Location wayp = new Location();
				wayp = LocationBundle.fromBundle(locations
						.getBundle(Location.POINT_TYPE[Location.WAYP]
								+ String.valueOf(i)));
				trip.getWaypoints().add(wayp);
			}
		}
		if (data.containsKey(Trip.PREFERENCES))
			trip.setPreferences(PrefsBundle.fromBundle(data
					.getBundle(Trip.PREFERENCES)));
